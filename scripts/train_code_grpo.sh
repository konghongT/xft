...
CWD=`dirname ${CWD}`
PWD=`dirname ${CWD}`
export LIVECODEBENCH_DATA_PATH=${LIVECODEBENCH_DATA_PATH:-/root/xft/data/eval/or1_data/eval/livecodebench/jsons}

model="deepseek-ai/DeepSeek-R1-Distill-Qwen-7B"

dataset="code_6k"
val_dataset="livecodebench"
...
total_training_steps_with_hint=0
...
PROFILE_STEPS="[1,2,5]" # or [] or nullqq
PROFILE_RANKS_ALL=True # or False
...
help() {
    ...
    echo "        --split_prompt"
    echo "               split content to user, system etc by rules. (default: false)"
...
val_data_dir="${PWD}/data/${val_dataset}"
...
PYTHONPATH=verl:. python -m xft.trainer.main_ppo \
    ...
    +data.trainer.split_prompt=False \
    +data.trainer.only_rl=False \
