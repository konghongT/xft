...
class RLHFDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.processor is not None:
            ...
            input_ids = model_inputs.pop("input_ids")
            attention_mask = model_inputs.pop("attention_mask")
            if "hint" in row_dict and not self.is_eval:
                ###############################
                # hint sampling policy
                ###############################
                ...
                if slice_length > 0:
                    hint_inputs = self.tokenizer(sliced_hint, return_tensors="pt", add_special_tokens=False)

        input_length = input_ids.shape[-1]
        if hint_inputs:
            hint_length = hint_inputs["input_ids"].shape[-1]
            input_ids = torch.cat((input_ids, hint_inputs["input_ids"]), dim=-1)
            attention_mask = torch.cat((attention_mask, hint_inputs["attention_mask"]), dim=-1)
        else:
            hint_length = 0
            hint_slice_prop = 0
